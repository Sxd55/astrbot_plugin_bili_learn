...
从 B 站**公开视频**自动学习短摘要，写入 AstrBot 官方知识库；用户发链接/BV 号时 AI 可**按需读取单个视频**并用自己的人设回复。Cookie 选填，不点赞、不评论、不私信、不投币、不改人格文件。

当前版本 `v1.9.1`。需要 AstrBot `>= 4.5.7`（官方知识库 + LLM 工具；兴趣词表格需要 `>= 4.10.4`）。纯标准库实现，无第三方依赖。
...
| `bili/tool.py` | `bilibili_read` FunctionTool |
