...
当前版本 `v1.12.0`。需要 AstrBot `>= 4.5.7`（官方知识库 + LLM 工具；兴趣词表格需要 `>= 4.10.4`）。纯标准库实现，无第三方依赖。
...
107 个测试，只用 Python 3.11+ 标准库，不联网。集成测试（23 个：真实 AstrBot 框架 + 假 KB/假 B 站/脚本化假模型，覆盖插件加载、全部命令、两个 LLM 工具、面板接口、配额/延期/汇总/审核全链路，以及任务分档、Token 预算闸、拒答重试；需 `pip install astrbot`）：
